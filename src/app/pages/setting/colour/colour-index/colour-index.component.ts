...
@Component({
  selector: 'app-colour-index',
  templateUrl: './colour-index.component.html',
  styleUrls: ['./colour-index.component.scss']
})
export class ColourIndexComponent implements OnInit, AfterViewInit {

  source: ServerDataSource | any;
  pageSize = 20;
  pageNumber = 1;
  totalRecords = 0;
  listUrl = environment.apiurl + 'v1/colour.php?_route=index';
  totalRows = 0;
  currentRows = 0;
  ...
  private baseRoute = 'v1/colour.php';
  ...
  constructor(
    ...
    this.titleService.setTitle('Colour');
  }
  ...
  ngAfterViewInit(): void {
    this.smartTable.custom.subscribe((dataObject: any) => {
      ...
      if (dataObject.action === 'editAction') {
        this.router.navigate(['/pages/colour/update/' + dataObject.data.id], { queryParams: { indexPage: this.pageNumber } });
    ...
    this.apiService.deleteRequest(this.baseRoute, {
      ...
      .subscribe(response => {
        if (response.body.status === 1) {
          this.toastrService.success('Colour deleted successfully!');
          this.source = this.getData();
        } else {
